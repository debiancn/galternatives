...
from . import logger, eprint, APPID, INFO, PACKAGE
...
class EditDialog(Gtk.Dialog):
    ...
    def add_row(self, button):
        ...
        new_row[0] = '<new>'
        self.slaves_model.append(new_row)
...
class GroupDialog(EditDialog):
    ...
    def __init__(self, group: AltGroup | None = None, *args, **kwargs):
        self.group = group
...
class OptionDialog(EditDialog):
    ...
    def __init__(
            self, group: AltGroup, option: AltOption | None = None,
            *args, **kwargs):
        self.group = group
        self.option = option
        ...
        if self.option is None:
            ...
            for slave_name in self.group[1:]:
                self.slaves_model.append([slave_name, None, None, None])
        else:
            self.set_title(
...
class MainWindow:
    ...
    def on_preferences_dialog_response(self, widget, response_id):
        ...
        if any(getattr(self.db, option) != path
               for option, path in self.altdb_paths.items()):
            self.load_db()
    ...
    def do_save(self, diff_cmds: list[list[str]] | None = None, autosave=False):
        """Save changes."""
        if diff_cmds is None:
            diff_cmds = self.db.compare(self.db_orig)
        self.main_window.set_sensitive(False)
        if self.debug > 1:
            logger.debug(
                'submit commands %s%s', diff_cmds,
                ' via polkit' if self.use_polkit else '')
        threading.Thread(target=lambda: GObject.idle_add(
            self.do_save_callback, diff_cmds, autosave, *self.db.commit(
                diff_cmds, 'pkexec' if self.use_polkit else None))).start()

    def do_save_callback(
            self, diff_cmds: list[list[str]], autosave: bool, returncode: int,
        ...
        if returncode:
            ...
            for cmd, result in zip_longest(
                    describe_cmds(diff_cmds), results, fillvalue=None):
    ...
    def show_group_window(self, group: AltGroup | None):
    ...
    def show_option_window(self, group: AltGroup, option: AltOption | None):
        window = OptionDialog(group, option, transient_for=self.main_window)
        window.present()
            ...
            if self.group:
                for row in model:
                    if row[0] == self.group.name:
                        ...
                        break
            return
        group = self.db[model.get_value(it, 0)]
        if group == self.group:
            return
...
class GAlternativesApp(Gtk.Application):
    ...
    def do_handle_local_options(self, options: GLib.VariantDict):
        ...
        if setColoredLogger:
            setColoredLogger(logger, self.debug)
        ...
        for option in AltDB.PATH_NAMES:
            ...
            if value:
                self.altdb_paths[option] = bytes(value).rstrip(b'\0').decode()

        return -1

    def do_local_command_line(self, arguments: list[str]):
        self.debug = 0
    ...
    def do_command_line(self, command_line: Gio.ApplicationCommandLine):
        ...
        if len(args) > 2:
            eprint(_('Error: Specifying more than one group is not allowed'))
            return 2
        ...
        if len(args) > 1:
            ...
            if self.target_group not in self._window.db:
                eprint(_('Error: No such group "{}"').format(self.target_group))
                self._window.destroy()
                return 1
